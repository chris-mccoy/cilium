...
func (n *linuxNodeHandler) enableSubnetIPsec(v4CIDR, v6CIDR []*net.IPNet) {
	...
	for _, cidr := range v4CIDR {
		...
		spi, err = ipsec.UpsertIPsecEndpoint(ipsecIPv4Wildcard, cidr, nil, ipsec.IPSecDirOut, zeroMark)
	...
	for _, cidr := range v6CIDR {
		...
		n.replaceNodeIPSecOutRoute(cidr)
		spi, err := ipsec.UpsertIPsecEndpoint(ipsecIPv6Wildcard, cidr, nil, ipsec.IPSecDirOut, zeroMark)
		...
		if n.nodeConfig.EncryptNode {
			...
			spi, err := ipsec.UpsertIPsecEndpoint(linkAddr, ipsecIPv6Wildcard, cidr, ipsec.IPSecDirIn, false)
